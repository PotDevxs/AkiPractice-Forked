...
import aki.saki.practice.adapter.ScoreboardAdapter
import aki.saki.practice.api.PracticeAPI
import aki.saki.practice.arena.Arena
import aki.saki.practice.arena.type.ArenaType
import aki.saki.practice.command.*
import aki.saki.practice.command.admin.*
import aki.saki.practice.constants.Constants
import aki.saki.practice.database.FlatFilePracticeDatabase
...
import aki.saki.practice.holograms.HologramManager
...
import aki.saki.practice.queue.task.QueueTask
import aki.saki.practice.task.*
import aki.saki.practice.utils.AnimatedTextManager
...
class PracticePlugin : JavaPlugin() {
    ...
    lateinit var database: PracticeDatabase
    ...
    override fun onEnable() {
        ...
        loadConfigFiles()
        MenuYamlSupport.extractDefaults(this)
        HotbarYamlLoader.reload(this)
        logger.info("Successfully loaded files!")

        database = buildDatabase()
        database.initialize().thenRun {
            logger.info("Database (${settingsFile.config.getString("DATABASE.TYPE") ?: "MONGODB"}) initialization completed")
            try {
                onDatabaseReady()
            } catch (e: Exception) {
                logger.severe("Failed to run onDatabaseReady: ${e.message}")
                e.printStackTrace()
            }
        }.exceptionally { ex ->
            logger.severe("Failed to initialize database: ${ex.message}")
            ex.printStackTrace()
            null
        }
    }
    ...
    private fun setupCommands() {
        ...
        val commands = mapOf(
            ...
            "ping" to PingCommand(),
            "adminmenu" to AdminYamlMenuCommand(),
            "eventmenu" to EventYamlMenuCommand(),
            "levelmenu" to LevelsYamlMenuCommand(),
            "hotbarmenu" to HotbarYamlMenuCommand()
        )
