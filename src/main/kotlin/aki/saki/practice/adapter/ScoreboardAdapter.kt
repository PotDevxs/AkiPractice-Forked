 ...
 * Date: 11/02/2026
 */
package aki.saki.practice.adapter
...
import aki.saki.practice.match.MatchType
import aki.saki.practice.match.impl.BedFightMatch
import aki.saki.practice.match.impl.BridgeMatch
import aki.saki.practice.match.impl.MLGRushMatch
import aki.saki.practice.match.impl.TeamMatch
...
class ScoreboardAdapter(private val configFile: ConfigFile) : AssembleAdapter {
    ...
    private val titleFrameIndex = AtomicInteger(0)
    private val titleCallCount = AtomicInteger(0)

    private fun getConfigString(path: String): String =
        configFile.getString("Scoreboard.${path.substringAfter(".")}").ifEmpty { configFile.getString(path) }

    private fun getConfigBoolean(path: String): Boolean =
        configFile.getBoolean("Scoreboard.${path.substringAfter(".")}") || configFile.getBoolean(path)

    private fun getConfigStringList(path: String): List<String> {
        val withScoreboard = "Scoreboard.${path.substringAfter(".")}"
        val list = configFile.getStringList(withScoreboard)
        return if (list.isNotEmpty()) list else configFile.getStringList(path)
    }

    private fun getConfigInt(path: String): Int {
        val withScoreboard = "Scoreboard.${path.substringAfter(".")}"
        val v = configFile.getInt(withScoreboard)
        return if (v != 0) v else configFile.getInt(path)
    }

    override fun getTitle(player: Player): String {
        val animated = getConfigBoolean("scoreboard.animated")
        val titleFrames = getConfigStringList("scoreboard.title-frames")
        if (animated && titleFrames.isNotEmpty()) {
            val interval = getConfigInt("scoreboard.title-frame-interval").coerceAtLeast(1)
            val calls = titleCallCount.incrementAndGet()
            if (calls % interval == 0) {
                titleFrameIndex.updateAndGet { (it + 1) % titleFrames.size }
            }
            return CC.translate(titleFrames[titleFrameIndex.get()])
        }
        val title = getConfigString("scoreboard.title").ifEmpty { configFile.getString("scoreboard.title") }
