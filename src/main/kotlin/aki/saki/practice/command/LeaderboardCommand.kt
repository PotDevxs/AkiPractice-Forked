...
class LeaderboardCommand {
    ...
    @Command(name = "setup-level", desc = "")
    @Require("practice.command.setup.leaderboards")
    fun leaderboardLevelSetup(@Sender sender: CommandSender) {
        val player = sender as Player
        PracticePlugin.instance.settingsFile.config.set(
            "HOLOGRAMS.LEVEL-LEADERBOARDS.LOCATION",
            LocationUtil.r(player.location)
        )
        PracticePlugin.instance.settingsFile.save()
        PracticePlugin.instance.leaderboards.refreshLevelHologram()
        player.sendMessage(CC.translate("&aHolograma &6TOP NÍVEL &adefinido aqui. A lista ordena por XP (maior nível em 1º) e atualiza sozinha."))
    }
}
