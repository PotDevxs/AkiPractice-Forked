...
class KitSettingsMenu(private val kit: Kit) : Menu() {
    ...
    override fun getTitle(player: Player): String {
        return "${CC.PRIMARY}Editando ${kit.name}"
    }
    ...
    override fun getButtons(player: Player): MutableMap<Int, Button> {
        ...
        buttons[0] = ToggleButton(kit, "Casual", Material.ENCHANTED_BOOK, { it.enabled }, { it.enabled = !it.enabled }, QueueType.UNRANKED)
        ...
        buttons[4] = ToggleButton(kit, "Competitivo", Material.DIAMOND_SWORD, { it.ranked }, { it.ranked = !it.ranked }, QueueType.RANKED)
        ...
        buttons[10] = ToggleButton(kit, "Fome", Material.COOKED_BEEF, { it.hunger }, { it.hunger = !it.hunger }, QueueType.UNRANKED)
        buttons[11] = ToggleButton(kit, "Regeneração", Material.BONE, { it.regeneration }, { it.regeneration = !it.regeneration }, QueueType.UNRANKED)
        buttons[12] = ToggleButton(kit, "Dano de queda", Material.FEATHER, { it.fallDamage }, { it.fallDamage = !it.fallDamage }, QueueType.UNRANKED)
    ...
    private class ToggleButton(
        ...
        override fun getButtonItem(player: Player?): ItemStack {
            ...
            return ItemBuilder(material)
                ...
                .lore(
                    listOf(
                        if (state) "${CC.GREEN}⚫ Ativado" else "${CC.RED}⚫ Ativado",
                        if (!state) "${CC.GREEN}⚫ Desativado" else "${CC.RED}⚫ Desativado"
                    )
                ).build()
        }
                    ...
                    .forEach { (_, queue) ->
                        queue.getQueueingPlayers().mapNotNull { PracticePlugin.instance.profileManager.findById(it.uuid) }
                            .forEach {
                                ...
                                it.player.sendMessage("${CC.RED}Você foi removido da fila.")
                            }
