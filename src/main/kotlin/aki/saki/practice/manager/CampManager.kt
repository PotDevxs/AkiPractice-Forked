...
object CampManager {
    ...
    fun saveCamp(camp: Camp) {
        try {
            ...
            PracticePlugin.instance.database.campUpsert(doc)
        } catch (_: Exception) { }
    }

    private fun deleteCamp(uuid: UUID) {
        try {
            PracticePlugin.instance.database.campDelete(uuid.toString())
        } catch (_: Exception) { }
    }
    ...
    fun load() {
        camps.clear()
        try {
            PracticePlugin.instance.database.campListAll().forEach { doc ->
