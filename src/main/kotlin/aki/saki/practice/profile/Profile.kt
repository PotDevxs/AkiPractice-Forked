...
class Profile(val uuid: UUID, var name: String? = null) : IProfile {
    ...
    override fun save(async: Boolean) {
        val document = toBson()
        PracticePlugin.instance.database.profileUpsert(document, async)
    }

    override fun load() {
        val document = PracticePlugin.instance.database.profileFindById(uuid.toString())
        if (document == null) {
    ...
    override fun delete() {
        ...
        PracticePlugin.instance.database.profileDelete(uuid.toString())
    }
