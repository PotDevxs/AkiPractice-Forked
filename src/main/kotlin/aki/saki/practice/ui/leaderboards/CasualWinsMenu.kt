...
class CasualWinsMenu(val p: PracticePlugin): Menu() {
    ...
    override fun getTitle(p0: Player?): String {
        return CC.translate("&7Casual Wins Leaderboards")
    }
    ...
    override fun getButtons(p0: Player): MutableMap<Int, Button> {
        ...
        for (kits in p.kitManager.kits.values) {
            ...
            val lore = listOf("${CC.PRIMARY}Your score: &f${p.profileManager.findById(p0.uniqueId)!!.getKitStatistic(kits.name)!!.wins}").toMutableList()
