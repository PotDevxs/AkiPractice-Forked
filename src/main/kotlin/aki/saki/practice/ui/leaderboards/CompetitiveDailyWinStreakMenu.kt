...
class CompetitiveDailyWinStreakMenu(val p: PracticePlugin): Menu() {
    ...
    override fun getButtons(p0: Player): MutableMap<Int, Button> {
        ...
        for (kits in p.kitManager.kits.values) {
            if (kits.ranked) {
                ...
                lore.addAll(p.leaderboards.getTopProfilesByKitCompetitiveDailyWins(kits).sortedByDescending { it.second }
                    .take(10)
