...
class LeaderboardRankedMenu(val p: PracticePlugin): Menu() {
            ...
            if (kits.ranked) {
                ...
                val lore = listOf(
                    "${CC.PRIMARY}Your score: &f${
                        p.profileManager.findById(p0.uniqueId)!!.getKitStatistic(kits.name)!!.elo
                    }"
                ).toMutableList()
                lore.addAll(p.leaderboards.getTopProfilesByKitElo(kits).sortedByDescending { it.second }
                    .take(10)
                    .mapIndexed { index, (profile, elo) ->
                        "${CC.PRIMARY}#<top>. &r<name> &7- &f<elo>"
                            .replace("<top>", (index + 1).toString())
                            .replace(
                                "<name>",
                                "${
